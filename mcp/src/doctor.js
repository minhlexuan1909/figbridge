...
async function listFigbridgeProcesses() {
  ...
  for (const line of stdout.split("\n")) {
    ...
    if (
      !/figbridge-mcp(\.js)?(\s|$)/.test(cmd) &&
      !/bin\/figbridge-mcp/.test(cmd)
    )
      continue;
  ...
  return new Promise((resolve) => {
    const req = http.get(
      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            resolve({ port, ok: true, body: JSON.parse(buf) });
          } catch {
...
export async function reapOrphans({ quiet = false } = {}) {
  const log = quiet
    ? () => {}
    : (...a) => process.stderr.write("[figbridge] " + a.join(" ") + "\n");
  let procs;
  try {
    procs = await listFigbridgeProcesses();
  } catch (e) {
    log("could not list processes:", e.message);
    return { found: 0, killed: 0 };
  }
  if (procs.length === 0) {
    log("no stale figbridge-mcp processes");
    return { found: 0, killed: 0 };
  }
  log(
    `found ${procs.length} figbridge-mcp process${
      procs.length === 1 ? "" : "es"
    }:`
  );
...
export async function runDoctor() {
  ...
  const live = probes.filter(
    (p) => p.ok && p.body && p.body.name === "figbridge-bridge"
  );
  const other = probes.filter(
    (p) => p.ok && (!p.body || p.body.name !== "figbridge-bridge")
  );
  ...
  if (live.length === 0) {
    ...
    process.stdout.write(
      `Live figbridge bridge${live.length > 1 ? "s" : ""}:\n`
    );
    for (const p of live) {
      const b = p.body;
      process.stdout.write(
        `  :${p.port}  clients=${b.clients ?? 0}  hasLatest=${
          b.hasLatest ? "yes" : "no"
        }\n`
      );
    }
  ...
  for (const p of other) {
    process.stdout.write(
      `  :${p.port}  responded but not figbridge (port conflict)\n`
    );
  }
  process.stdout.write("\n");
  ...
  if (found === 0) {
    ...
    process.stdout.write(
      `Reaped ${killed}/${found} orphan figbridge-mcp process${
        found === 1 ? "" : "es"
      }.\n`
    );
  }
  ...
  if (found > 0) {
    process.stdout.write(
      "  • Restart Claude Desktop so it respawns a fresh figbridge-mcp on :7331.\n"
    );
  } else if (live.length === 0) {
    process.stdout.write(
      "  • Restart Claude Desktop (or run `npx figbridge-mcp init` if never installed).\n"
    );
  } else {
    process.stdout.write(
      "  • All good. Open the Figbridge plugin in Figma and toggle Live bridge on.\n"
    );
  }
}
